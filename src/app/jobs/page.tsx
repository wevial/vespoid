  ...
  return (
    <main className="vespoid-shell mx-auto flex max-w-7xl flex-col gap-6 p-6 md:p-10">
      <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          <Link className="text-sm text-berry hover:text-berry" href="/">← Dashboard</Link>
          <h1 className="vespoid-heading mt-2 text-3xl font-semibold">Jobs</h1>
        </div>
      </header>

      <section className="vespoid-panel vespoid-filter-panel grid gap-4 rounded-2xl p-5 sm:grid-cols-2 lg:grid-cols-3">
        <label className="vespoid-filter-label">Search
          <input value={search} onChange={(e) => setFilter({ search: e.target.value })} placeholder="Search title/company/preview" className="vespoid-input rounded-xl px-3 py-2 text-sm outline-none" />
        </label>
        <label className="vespoid-filter-label">Source
          <select value={source} onChange={(e) => setFilter({ source: e.target.value as typeof source })} className="vespoid-input rounded-xl px-3 py-2 text-sm outline-none">
        ...
        <label className="vespoid-filter-label">Status
          <select value={status} onChange={(e) => setFilter({ status: e.target.value as typeof status })} className="vespoid-input rounded-xl px-3 py-2 text-sm outline-none">
          <option value="">Any status</option>
          {JOB_LIST_STATUS_FILTER_OPTIONS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
        </select>
        </label>
        <label className="vespoid-filter-label">Area
          <select value={area} onChange={(e) => setFilter({ area: e.target.value as JobAreaFilter })} className="vespoid-input rounded-xl px-3 py-2 text-sm outline-none" aria-label="Filter by area">
        ...
        <label className="vespoid-filter-label">Working style
          <input value={remote} onChange={(e) => setFilter({ remote: e.target.value })} placeholder="remote / hybrid / onsite" className="vespoid-input rounded-xl px-3 py-2 text-sm outline-none" />
        </label>
        <label className="vespoid-filter-label">Sort
          <select value={sort} onChange={(e) => setFilter({ sort: e.target.value as JobSortOption })} className="vespoid-input rounded-xl px-3 py-2 text-sm outline-none" aria-label="Sort jobs">
      ...
      {jobs === undefined ? (
        <div className="vespoid-panel rounded-2xl p-8 text-muted">Loading jobs…</div>
      ) : sortedJobs && sortedJobs.length === 0 ? (
        <div className="vespoid-panel rounded-2xl p-8 text-muted">No jobs match these filters.</div>
      ) : (
        <section className="vespoid-panel vespoid-panel-featured overflow-hidden rounded-2xl">
          <div className="hidden grid-cols-12 gap-3 border-b border-line px-4 py-3 text-xs uppercase tracking-wide text-muted md:grid">
          ...
          <div className="vespoid-divider divide-y divide-line">
            {sortedJobs?.map((job) => (
              <div
                ...
                className="vespoid-row grid grid-cols-1 gap-3 px-4 py-4 text-sm md:grid-cols-12 lg:cursor-pointer"
              >
                <Link href={`/jobs/${job._id}`} onClick={(event) => handleJobTitleClick(event, job._id)} className="md:col-span-3">
                  <strong className="block text-ink">{job.title}</strong>
                  <span className="text-muted">{job.company} · {job.location ?? "Unknown"}</span>
                  {job.preferenceReasons && job.preferenceReasons.length > 0 ? (
                    <span className="mt-1 block text-xs text-berry">Personalized {job.personalizedScore}: {job.preferenceReasons.slice(0, 2).join(" · ")}</span>
                  ) : null}
                  {job.fitReasons && job.fitReasons.length > 0 ? (
                    <span className="mt-1 block text-xs text-muted">Fit {job.fitScore ?? "—"}: {job.fitReasons.slice(0, 3).join(" · ")}</span>
                  ) : null}
                </Link>
                <span className="text-berry md:col-span-2"><span className="md:hidden text-muted">Salary: </span>{formatSalaryPreview(job.salaryRange)}</span>
                <div className="flex flex-wrap items-start gap-2 md:col-span-2">
                  <span className="text-muted"><span className="md:hidden text-muted">Source: </span>{SOURCE_LABELS[job.source] ?? job.source}</span>
                  <button
                    type="button"
                    onClick={() => togglePreview(job._id)}
                    className={`hidden rounded-xl border px-3 py-1 text-xs font-semibold transition lg:inline-flex ${
                      previewJob?._id === job._id
                        ? "border-line bg-lilac text-ink ring-1 ring-berry"
                        : "border-line bg-background text-muted hover:border-line hover:text-ink"
                    }`}
                ...
                <span className="text-muted md:col-span-1"><span className="md:hidden text-muted">Remote: </span>{job.remoteStatus ?? "—"}</span>
                <span className="text-muted md:col-span-1"><span className="md:hidden text-muted">Discovered: </span>{formatDate(job.discoveredAt)}</span>
      ...
      {jobs !== undefined && !isDone ? (
        <div className="flex justify-center">
          <button
            ...
            className="vespoid-button rounded-xl px-5 py-3 text-sm font-semibold disabled:cursor-wait disabled:opacity-55"
          >
      ...
      <aside
        ...
        className={`fixed bottom-0 right-0 top-0 z-40 hidden transform border-l border-line vespoid-preview lg:block ${
      ...
      >
        <button
          ...
          className="absolute -left-1 top-0 z-50 hidden h-full w-3 cursor-col-resize touch-none border-l border-line transition hover:border-line lg:block"
        />
        {previewJob ? (
          <div className="flex h-full flex-col">
            <div className="border-b border-line p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="vespoid-eyebrow text-xs uppercase tracking-[0.25em]">{SOURCE_LABELS[previewJob.source] ?? previewJob.source}</p>
                  <h2 className="vespoid-heading mt-2 text-xl font-semibold leading-tight">{previewJob.title}</h2>
                  <p className="mt-2 text-sm text-muted">{previewJob.company} · {previewJob.location ?? "Unknown"}</p>
                </div>
                <button type="button" onClick={() => setPreviewJobId(undefined)} className="vespoid-ghost rounded-xl px-3 py-1 text-sm" aria-label="Close preview panel">
                  Close
                </button>
              </div>
              <div className="mt-4 flex flex-wrap gap-2">
                <Link href={`/jobs/${previewJob._id}`} onClick={rememberScrollPosition} className="vespoid-ghost rounded-xl px-4 py-2 text-xs font-semibold">
                  Full page →
                </Link>
                <a href={previewJob.url} target="_blank" rel="noopener noreferrer" className="vespoid-ghost rounded-xl px-4 py-2 text-xs font-semibold">
                  Original ↗
                </a>
              </div>
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-y-auto p-5">
              <dl className="grid gap-3 rounded-xl border border-line bg-background p-4 text-sm sm:grid-cols-2">
                ...
                <div><dt className="text-xs text-muted">Personalized</dt><dd className="text-berry">{previewJob.personalizedScore ?? "Unknown"}</dd></div>
                <div><dt className="text-xs text-muted">Discovered</dt><dd className="text-ink">{formatDate(previewJob.discoveredAt)}</dd></div>
              </dl>

              {previewJob.preferenceReasons && previewJob.preferenceReasons.length > 0 ? (
                <div className="mt-4 rounded-xl border border-line bg-lilac p-3 text-sm text-berry">
              ...
              <div className="mt-5 flex min-h-[520px] flex-1 flex-col rounded-xl border border-line bg-background p-3">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <h3 className="text-sm font-semibold text-ink">Embedded listing</h3>
                  <a className="text-xs text-berry hover:text-berry" href={previewJob.url} target="_blank" rel="noopener noreferrer">Open ↗</a>
                </div>
                <iframe src={previewJob.url} title={`${previewJob.title} preview`} sandbox="allow-scripts" className="min-h-[420px] flex-1 rounded-xl border border-line bg-white" />
                <p className="mt-2 text-xs text-muted">Some job sites block iframe previews; use Original or Full page if blank.</p>
              </div>
            </div>
          </div>
        ) : null}
